#nuScenesのLiDAR点群から動的物体を除去して静的マップ＋反射強度画像を作る」処理を**改良版（より厳密に“実際に動いている物体”を除去するように改善）
import numpy as np
import copy
...
# =========================================================
# 1. Boxの中に点が含まれているかどうか判定する関数 (変わらず)
# =========================================================
...
# =========================================================
# 2. "動的物体"かどうか (カテゴリがvehicle/human/movable_object)
#    ただし、これだけだと"moving"以外も含んでしまう
# =========================================================
...
# =========================================================
# 3. "本当に動いている(moving)"かどうか
#    属性に "moving" が含まれるかをチェック
# =========================================================
...
# 4. Boxを拡大するユーティリティ
# =========================================================
def inflate_box(original_box: Box, scale_factor: float = 2.0) -> Box:
    """
    オリジナルBoxをdeepcopyして、幅・奥行き・高さ (w, l, h) を scale_factor 倍に拡張。
    """
...
# =====================================================================
# センサー座標系→車両座標系→世界座標系の処理と点群の統合 (動的物体"moving"除去)
# =====================================================================
for sample_number in range(start_sample_number, end_sample_number + 1):
    ...
    # 近すぎる点を除外 (threshold_distance=2.0m)
    ...
    # ------------------------------------------------
    # (追加) 動的物体かつ "moving" 属性のBoxを拡大して除去
    # ------------------------------------------------
    ...
    for box in boxes:
        if is_dynamic_category(box.name) and is_really_moving(box.token, nusc):
            # Boxを2倍に拡大
            inflated_box = inflate_box(box, scale_factor=2.0)
            mask_in_box = points_in_box_3d(inflated_box, lidar_points_transformed[:, :3])
